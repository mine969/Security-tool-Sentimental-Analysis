...
def calculate_threat_level(severity_score, risk_level):
    """
    Calculate threat level based on severity score and risk level.
    ...
    try:
        ...
        return 'Medium'  # Default to Medium if data is invalid

def main():
    print("="*60)
    print("CYBERSECURITY THREAT ANALYSIS WITH THREAT LEVEL")
    print("="*60)
    ...
    try:
        ...
        print("Error: cyber_security.csv not found. Please ensure the dataset exists.")
        return

    print(f"Dataset shape: {df.shape}")
    
    # Check for required columns
    required_cols = ['Cleaned Threat Description', 'Severity Score', 'Risk Level Prediction']
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        print(f"Error: Missing required columns: {missing_cols}")
        return

    # Calculate Threat Level on-the-fly from existing columns
    print("\nCalculating Threat Level from Severity Score and Risk Level Prediction...")
    df['Threat Level'] = df.apply(
        lambda row: calculate_threat_level(row['Severity Score'], row['Risk Level Prediction']),
        axis=1
    )
    print("✓ Threat Level calculated successfully")
